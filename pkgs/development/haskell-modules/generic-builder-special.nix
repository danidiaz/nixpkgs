...
, preexistingDist ? null
, preexistingDistNewstyle ? null
, buildTarget ? ""
...
, enableSeparateDistNewstyleOutput ? false
...
stdenv.mkDerivation ({
  ...
  outputs = [ "out" ]
         ...
         ++ (optional enableSeparateDistNewstyleOutput "distNewstyle");
  ...
  ''
  +
  (if (preexistingDist != null) then ''

  echo cp -R ${preexistingDist} dist
  cp -R ${preexistingDist} dist
  ''
  else
  ''
  ''
  ...
  +
  (if (preexistingDistNewstyle != null) then ''
  echo cp -R ${preexistingDistNewstyle} dist-newstyle
  cp -R ${preexistingDistNewstyle} dist-newstyle
  ''
  else
  ''
  ''
  ...
  buildPhase = ''
    runHook preBuild
    echo "The setup command is: " ${setupCommand}
    echo "---- before"
    ls dist/build
    ls dist-newstyle/
    ls dist-newstyle/cache
    ls dist-newstyle/build/
    ls dist-newstyle/cache
    ls dist-newstyle/build/x86_64-linux/
    ls dist-newstyle/build/x86_64-linux/ghc-9.0.2/
    ls dist-newstyle/build/x86_64-linux/ghc-9.0.2/foo-1.0.0.0/
    ls dist-newstyle/build/x86_64-linux/ghc-9.0.2/foo-1.0.0.0/x/
    ${setupCommand} build --verbose=3 ${buildTarget}${crossCabalFlagsString}${buildFlagsString}
    echo "----"
    ls dist/build
    ls dist-newstyle/build/x86_64-linux/ghc-9.0.2/foo-1.0.0.0/x/
    echo "----"
  ...
  installPhase = ''
    ...
    ${optionalString enableSeparateDistNewstyleOutput ''
    mkdir -p $distNewstyle
    cp -R dist-newstyle/* $distNewstyle
    ''}
